...
describe("API de postagens", () => {
  ...
  it("deve retornar 401 ao criar postagem sem token", async () => {
  ...
  it("deve retornar 401 ao criar postagem com token inválido", async () => {
    const resposta = await request(app)
      .post("/posts")
      .set(
        "Authorization",
  ...
  it("deve criar uma nova postagem", async () => {
    const resposta = await request(app)
      ...
      .send({
        ...
        conteudo:
          "Conteúdo do teste automatizado.",
        autor: "Andy"
      });

    expect(resposta.status).toBe(201);
    expect(resposta.body).toHaveProperty("id");
    expect(resposta.body.titulo).toBe(
      "Teste de criação"
    );
  ...
  it("não deve criar postagem sem os campos obrigatórios", async () => {
    ...
    expect(resposta.body).toEqual({
      mensagem:
        "Título, conteúdo e autor são obrigatórios."
    });
  });
  ...
  it("deve listar as postagens cadastradas", async () => {
    ...
    expect(Array.isArray(resposta.body)).toBe(
      true
    );
    expect(resposta.body).toHaveLength(1);
    expect(resposta.body[0].titulo).toBe(
      "Post para listagem"
    );
  });
